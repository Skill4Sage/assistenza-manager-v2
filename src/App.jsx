...
// --- COSTANTI E DATI ---
const PRIORITIES = {
  1: { name: "Critica", color: "#22c55e", bgColor: "#f0fdf4" },
  2: { name: "Urgente", color: "#3b82f6", bgColor: "#eff6ff" },
  3: { name: "Pianificabile", color: "#64748b", bgColor: "#f8fafc" },
  4: { name: "Bassa", color: "#06b6d4", bgColor: "#ecfeff" },
};

const SLOT_DURATION = 15;
const STORAGE_KEY = 'assistenza-manager-v2-stable-storage';

// --- COMPONENTE TASK ITEM ---
const TaskItem = React.memo(({ task, onToggleComplete, onDragStart, onRemove }) => {
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 p-4 md:p-8">
      <style>{`
        @media print {
          ...
          .print-page-1 { page-break-after: always; display: block !important; }
      ...
      <header className="max-w-7xl mx-auto flex flex-wrap items-center justify-between mb-8 gap-4 no-print">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-bold">Assistenza Manager V2</h1>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={prepareDay} className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-xl font-bold text-xs">
      ...
      <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 no-print">
        <section onDragOver={(e) => onDragOver(e, 'inbox')} onDrop={(e) => onDrop(e, 'inbox')} className="lg:col-span-3">
          <h2 className="flex items-center gap-2 font-bold text-slate-400 uppercase text-[10px] px-2 mb-4"><Book size={14} /> Inbox</h2>
          {tasks.filter(t => t.status !== 'today' && t.status !== 'done').map(task => (
            <TaskItem key={task.id} task={task} onToggleComplete={(id) => setTasks(prev => prev.map(t => t.id === id ? {...t, status: 'done'} : t))} onDragStart={onDragStart} onRemove={(id) => setTasks(prev => prev.filter(t => t.id !== id))} />
          ))}
        </section>
        ...
        <section className="lg:col-span-6 space-y-6">
          <div onDragOver={(e) => onDragOver(e, 'today')} onDrop={(e) => onDrop(e, 'today')} className={`bg-white rounded-3xl p-5 shadow-sm border-2 border-dashed transition-all ${dragOver === 'today' ? 'border-orange-400 bg-orange-50' : 'border-slate-100'}`}>
            <h2 className="flex items-center gap-2 font-bold text-orange-600 mb-4 text-xs uppercase"><Flame size={16} /> Fuochi del Giorno</h2>
          ...
          <div className="bg-white rounded-3xl p-5 shadow-sm">
            <h2 className="flex items-center gap-2 font-bold text-slate-400 mb-4 uppercase text-[10px]"><Clock size={14} /> Agenda Operativa</h2>
              ...
              {timeSlots.map(slot => {
                ...
                return (
                  <div key={timeStr} onDragOver={(e) => onDragOver(e, `slot-${timeStr}`)} onDrop={(e) => onDrop(e, `slot-${timeStr}`)}
                    className={`flex items-center gap-4 p-2 transition-colors ${isFullHour ? 'hour-row' : 'slot-row'}`}>
                    ...
                    <div className="flex-grow min-h-[24px]">
                      {task && (
                        <div className="bg-white border-l-4 shadow-sm text-[11px] p-1.5 rounded-lg flex justify-between items-center" style={{ borderLeftColor: PRIORITIES[task.priority].color }}>
                          <span className="font-bold truncate text-slate-700">{task.title}</span>
                          <button onClick={() => setScheduledTasks(prev => prev.filter(st => st.taskId !== task.id))}><X size={12}/></button>
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <section className="lg:col-span-3">
          <div className="bg-white rounded-3xl p-6 shadow-sm border h-full">
            <h3 className="font-bold text-slate-700 mb-4 text-sm flex items-center gap-2"><ClipboardList size={16}/> Note Strategiche</h3>
            <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full h-[60vh] bg-slate-50 rounded-2xl p-4 text-xs border-none resize-none" placeholder="Appunti liberi..." />
          </div>
        </section>
      </main>
      ...
      <div className="hidden print:block text-slate-900 bg-white">
        <div className="print-page-1 px-8 pt-4">
          <div className="flex justify-between items-end border-b-4 border-slate-900 pb-2 mb-6 uppercase">
            <h1 className="text-xl font-black">Piano Giornaliero</h1>
        ...
        <div className="print-page-2 px-8 pt-8">
          <h1 className="text-lg font-black mb-6 border-b-4 border-slate-900 pb-1 uppercase">Dettaglio Attività & Note</h1>
      ...
      {isAddingTask && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg p-8">
            <h2 className="text-xl font-bold mb-6">Nuova Richiesta</h2>
            <form onSubmit={(e) => { e.preventDefault(); const fd = new FormData(e.target); addTask({desc: fd.get('desc'), priority: fd.get('priority'), deadline: fd.get('deadline')}); }}>
              <textarea name="desc" required className="w-full rounded-2xl border-slate-200 h-44 text-sm p-4 mb-4" placeholder="Testo del problema..." />
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" className="flex-grow bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-lg">Salva</button>
              </div>
            </form>
          </div>
        </div>
